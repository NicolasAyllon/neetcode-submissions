...
private:
    // Invariant:
    // subset contains elements chosen from nums and we are deciding whether to include
    // nums[i] in the current subset.
    ...
    void backtrack(const std::vector<int> nums, int i, 
        ...
        // Base Case:
        // We've made a decision for all vals in input array whether to include it (yes/no)
        // We've created a subset, so add it to the result of subsets
        if (i >= nums.size()) {
        ...
        // Case 2.
        // We choose not to include nums[i] and this is the last copy of nums[i] we'll ever
        // include. To get to the next unique value, we advance i until
        // nums[i + 1] is a different value from nums[i].
