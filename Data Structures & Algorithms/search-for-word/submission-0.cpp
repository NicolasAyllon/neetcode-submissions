...
public:
    bool exist(vector<vector<char>>& board, string word) {
        ...
        // Try to find 'word' at every position on the board.
...
private:
    ...
    bool dfs(const vector<vector<char>>& board, int i, int j, const string& word, int k, vector<vector<bool>>& visited) {
        ...
        // Base case: Char at this position on the board doesn't match word[k]
        if (board[i][j] != word[k]) return false;
        // Base case: Already visited
        if (visited[i][j]) return false;

        // board[i][j] == word[k]
        // We can explore all unvisited spaces up, down, left, and right
        // And increase the index k, since we've found word[0, k] or equivalently word[0, k+1)

        // Mark visited
        visited[i][j] = true;
        // Explore
        bool result = dfs(board, i + 1, j, word, k + 1, visited) ||
