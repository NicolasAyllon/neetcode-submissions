...
public:
    ...
    vector<int> findOrder(int numCourses, vector<vector<int>>& prerequisites) {
        ...
        // If some nodes were not added to the output, it was because their indegree never dropped to 0.
        // The un-added nodes contain a cycle, so we return an empty list in that case.
        if (output.size() < n)
            output.clear();
